...
public class PlayFieldTypes
{
    public static final Supplier<PlayFieldType> DEFAULT = () -> new PlayFieldType(8000)
            ...
            .newInteraction().icon(
                    ActionIcons.TO_TOP_OF_DECK_FD.get())
                    .interactorUnequals(ZoneTypes.EXTRA_DECK.get())
                    .interactorCardNotNullNoToken()
                    .interacteeEquals(ZoneTypes.EXTRA_DECK.get())
                    .interaction(
                            (player, interactor, card, interactee) ->
                            new MoveTopAction(ActionTypes.MOVE_ON_TOP.get(), interactor, card, interactee, CardPosition.FD, player))
                            .playerAndInteractorSameOwner()
                            .interactorAndInteracteeSameOwner()
                            .addInteraction()
