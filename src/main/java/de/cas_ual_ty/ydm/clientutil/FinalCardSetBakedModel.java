...
public class FinalCardSetBakedModel implements BakedModel
{
    ...
    public List<BakedQuad> getQuads(BlockState state, Direction side, RandomSource rand)
    {
        ...
        {
            CardSet set = CardSetBaseItem.getCardSet(activeItemStack);
    ...
    public BakedModel applyTransform(ItemDisplayContext context, PoseStack mat, boolean applyLeftHandTransform)
    {
        switch(context)
        {
            ...
            case GROUND:
                ...
                break;
            case FIXED:
                mat.mulPose(new Quaternionf().rotationAxis((float)Math.PI, new Vector3f(Direction.UP.step())));
                break;
            default:
                break;
        }
        
        BakedModel.super.applyTransform(context, mat, applyLeftHandTransform);
