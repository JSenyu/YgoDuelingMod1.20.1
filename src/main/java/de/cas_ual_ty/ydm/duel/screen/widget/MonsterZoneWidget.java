...
{
    public MonsterZoneWidget(Zone zone,
                             IDuelScreenContext context,
                             Consumer<ZoneWidget> onPress,
                             ITooltip tooltip) {
        // 1) 用 Button.builder(...) 来取代旧的 super(...) 签名
    ...
    public DuelCard renderCards(GuiGraphics gui, int mouseX, int mouseY)
    {
        ...
        if(cardsAmount == 1)
        {
            ...
            if(drawCard(gui, c, renderX, renderY, renderWidth, renderHeight, mouseX, mouseY, newHoverX, hoverY, hoverWidth, hoverHeight))
            {
                hoveredCard = c;
                hoverX = newHoverX;
            }
        ...
        if(topCardInDef)
        {
            ...
            if(drawCard(gui, c, renderX, renderY, renderWidth, renderHeight, mouseX, mouseY, newHoverX, newHoverY, newHoverWidth, newHoverHeight))
            {
                hoveredCard = c;
                hoverX = newHoverX;
                hoverY = newHoverY;
