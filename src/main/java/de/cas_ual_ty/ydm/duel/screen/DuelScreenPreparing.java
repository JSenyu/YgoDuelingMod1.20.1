...
//            addRenderableWidget(chooseDeckButton = new ImprovedButton(x - chooseWidth / 2, topPos + imageHeight - 20 - 10, chooseWidth, 20, Component.translatable("container." + YDM.MOD_ID + ".duel.choose_deck"), (button) -> chooseDeckClicked(), this::chooseDeckTooltip));
            ...
            addRenderableWidget(prevDeckWidget = new ItemStackWidget(x - 16 - 16, topPos + imageHeight - 20 - 10 - 5 - 16 - 8, 16, minecraft.getItemRenderer(), CardRenderUtil.getInfoCardBack()));
            addRenderableWidget(activeDeckWidget = new ItemStackWidget(x - 16, topPos + imageHeight - 20 - 10 - 5 - 32, 32, minecraft.getItemRenderer(), CardRenderUtil.getInfoCardBack()));
            addRenderableWidget(nextDeckWidget = new ItemStackWidget(x - 16 + 32, topPos + imageHeight - 20 - 10 - 5 - 16 - 8, 16, minecraft.getItemRenderer(), CardRenderUtil.getInfoCardBack()));
    ...
    {
        gui.drawString(font ,"Choose your decks...",  8, 6, 0x404040, false);
        ...
        if(role == PlayerRole.PLAYER1 || role == PlayerRole.PLAYER2)
        {
            ...
            {
                drawActiveDeckForeground(gui, mouseX, mouseY);
            }
            else
            {
                ...
                gui.drawString(font, text, (int) ((imageWidth - width) / 2F), (int) ((this.height - height) / 2F), 0x404040, false);
            }
        }
        else
        {
            ...
            int width = font.width(text);
            int height = font.lineHeight;
            gui.drawString(font, text, (int) ((imageWidth - width) / 2F), (int) ((imageHeight - height) / 2F), 0x404040, false);
        }
    }
    ...
    protected void renderBg(GuiGraphics gui, float partialTicks, int mouseX, int mouseY)
    {
        ...
        if(renderDeckChoosing())
        {
            drawActiveDeckBackground(gui, partialTicks, mouseX, mouseY);
        }
    }
    
    protected void drawActiveDeckForeground(GuiGraphics gui, int mouseX, int mouseY)
    {
        ...
        if(h != DeckWrapper.DUMMY)
        {
            ...
            if(d != null && d != DeckHolder.DUMMY)
            {
                ...
                // main deck
                //drawString
                gui.drawString(font, Component.translatable("container.ydm.deck_box.main").append(" " + d.getMainDeckSize() + "/" + DeckHolder.MAIN_DECK_SIZE), guiLeft + 8, guiTop + 6, 0x404040, false);
                
                // extra deck
                //drawString
                gui.drawString(font, Component.translatable("container.ydm.deck_box.extra").append(" " + d.getExtraDeckSize() + "/" + DeckHolder.EXTRA_DECK_SIZE), guiLeft + 8, guiTop + 92, 0x404040, false);
                
                // side deck
                //drawString
                gui.drawString(font, Component.translatable("container.ydm.deck_box.side").append(" " + d.getSideDeckSize() + "/" + DeckHolder.SIDE_DECK_SIZE), guiLeft + 8, guiTop + 124, 0x404040, false);
...
//                            CardRenderUtil.bindMainResourceLocation(c);
                            YdmBlitUtil.fullBlit(gui, c.getMainImageResourceLocation(),guiLeft + offX, guiTop + offY, 16, 16);
                            
                            if(mouseX >= offX && mouseX < offX + size && mouseY >= offY && mouseY < offY + size)
                            {
                                ScreenUtil.renderHoverRect(gui, guiLeft + offX, guiTop + offY, 16, 16);
                                renderCardInfoForeground(gui, c, actualGuiLeft);
                            }
                        }
...
//                        CardRenderUtil.bindMainResourceLocation(c);
    ...
    protected void drawActiveDeckBackground(GuiGraphics gui, float partialTicks, int mouseX, int mouseY)
    {
        DeckWrapper h = getActiveDeckWrapper();
...
//                YdmBlitUtil.blit(gui, guiLeft, guiTop, xSize, ySize, 0, 0,
//                        xSize, ySize, 512, 256);
//                YdmBlitUtil.blit(gui, guiLeft, guiTop + ySize, xSize, 7, 0, 243,
//                        xSize, 7, 512, 256);

                gui.blit(DECK_BACKGROUND_GUI_TEXTURE, guiLeft, guiTop, xSize, ySize, 0, 0,
                        xSize, ySize, 512, 256);
                gui.blit(DECK_BACKGROUND_GUI_TEXTURE, guiLeft, guiTop + ySize, xSize, 7, 0, 243,
                        xSize, 7, 512, 256);
            }
        }
    }
    ...
    public void renderCardInfoForeground(GuiGraphics gui, CardHolder c)
    {
        renderCardInfoForeground(gui, c, leftPos);
    }
    
    public void renderCardInfoForeground(GuiGraphics gui, CardHolder c, int width)
    {
        gui.pose().pushPose();

        gui.pose().translate(-leftPos, -topPos, 0D);
        CardRenderUtil.renderCardInfo(gui, c, width);
    ...
    protected void chooseDeckTooltip(AbstractWidget w, GuiGraphics gui, int mouseX, int mouseY)
    {
//        renderTooltip(gui, Component.translatable("container.ydm.duel.choose_deck"), mouseX, mouseY);
    }
