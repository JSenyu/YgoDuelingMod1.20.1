...
//                event.addSprite(card.getItemImageResourceLocation(imageIndex));
                event.getAtlas().getTextureLocations().add(card.getItemImageResourceLocation(imageIndex));
            });
        }
...
//                    event.addSprite(set.getItemImageResourceLocation());
                    event.getAtlas().getTextureLocations().add(set.getItemImageResourceLocation());
                }
            }
        }
    }
...
//        event.getModels().put(key, new CardSetBakedModel(event.getModelManager().getModel(key)));
        event.getModelBakery().getBakedTopLevelModels().put(key, new CardSetBakedModel(event.getModelManager().getModel(key)));
...
//        event.getModels().put(key, new CardSetBakedModel(event.getModelManager().getModel(key)));
        ...
        if(event.getScreen() instanceof AbstractContainerScreen<?> containerScreen)
        {
            ...
            if(containerScreen.getSlotUnderMouse() != null && !containerScreen.getSlotUnderMouse().getItem().isEmpty())
            {
                ...
                if(itemStack.getItem() == YdmItems.CARD.get()) {
                    CardRenderUtil.renderCardInfo(event.getGuiGraphics(), CardItem.getCardHolder(itemStack), containerScreen);
                }
                else if(itemStack.getItem() == YdmItems.SET.get()) {
                    renderSetInfo(event.getGuiGraphics(), CardSetBaseItem.getCardSet(itemStack), containerScreen.getGuiLeft());
                }
                else if(itemStack.getItem() == YdmItems.OPENED_SET.get()) {
                    renderSetInfo(event.getGuiGraphics(), CardSetBaseItem.getCardSet(itemStack), containerScreen.getGuiLeft());
                }
                else if(itemStack.getItem() instanceof CardSleevesItem) {
                    renderSleevesInfo(event.getGuiGraphics(), ((CardSleevesItem) itemStack.getItem()).sleeves, containerScreen.getGuiLeft());
                }
            }
        }
    }
    ...
    private void renderGameOverlayPost(RenderGuiOverlayEvent.Post event)
    {
        ...
        if(getClientPlayer() != null && ClientProxy.getMinecraft().screen == null)
        {
            ...
            {
                CardRenderUtil.renderCardInfo(event.getGuiGraphics(), CardItem.getCardHolder(player.getMainHandItem()));
            }
            else if(player.getMainHandItem().getItem() == YdmItems.SET.get())
            {
                renderSetInfo(event.getGuiGraphics(), CardSetBaseItem.getCardSet(player.getMainHandItem()));
            }
            ...
            {
                renderSleevesInfo(event.getGuiGraphics(), ((CardSleevesItem) player.getMainHandItem().getItem()).sleeves);
            }
            else if(player.getOffhandItem().getItem() == YdmItems.CARD.get())
            {
                CardRenderUtil.renderCardInfo(event.getGuiGraphics(), CardItem.getCardHolder(player.getOffhandItem()));
            }
            else if(player.getOffhandItem().getItem() == YdmItems.SET.get())
            {
                renderSetInfo(event.getGuiGraphics(), CardSetBaseItem.getCardSet(player.getOffhandItem()));
            }
    ...
    private void renderSetInfo(GuiGraphics gui, CardSet set)
    {
        renderSetInfo(gui, set, 150);
    }
    
    private void renderSetInfo(GuiGraphics gui, CardSet set, int width)
    {
        if(set == null) return;
...
//        RenderSystem.setShaderTexture(0, set.getInfoImageResourceLocation());
        YdmBlitUtil.fullBlit(gui, set.getInfoImageResourceLocation(), x, margin, imageSize, imageSize);
//        gui.blit(set.getInfoImageResourceLocation(), x, margin, imageSize, imageSize, imageSize, imageSize);
    ...
    private void renderSleevesInfo(GuiGraphics gui, CardSleevesType sleeves)
    {
        renderSleevesInfo(gui, sleeves, 150);
    }
    
    private void renderSleevesInfo(GuiGraphics gui, CardSleevesType sleeves, int width)
    {
        if(sleeves == null)
        {
            return;
...
//        RenderSystem.setShaderTexture(0, sleeves.getMainRL(ClientProxy.activeCardInfoImageSize));
        YdmBlitUtil.fullBlit(gui, sleeves.getMainRL(ClientProxy.activeCardInfoImageSize), x, margin, imageSize, imageSize);
        ...
        ScreenUtil.drawSplitString(gui, fontRenderer, ImmutableList.of(Component.translatable("item.ydm." + sleeves.getResourceName())), margin, imageSize * 2 + margin * 2, maxWidth, 0xFFFFFF);
    ...
    {
        if(!event.isCanceled() && event.getMessage() != null && !event.getMessage().getString().isEmpty() && !ClientProxy.getMinecraft().isBlocked(event.getSender()))
        {
